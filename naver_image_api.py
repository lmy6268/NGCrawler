...
def saveImage(imgUrl: str, keyword: str, idx: int, savePath: str):
    ...
    try:
        ...
        with open("./log.txt", 'a' if not os.path.exists("./log.txt") else 'w') as f:
            f.write(imgUrl)
            f.close()
...
def doCrawl(driver: webdriver, keywords: str, baseUrl: str, forderPath: str):
    ...
    """
    #다운받는 이미지 목록을 저장해두고, 중복저장을 막음.
    try:
        p = open("./imgUrl.txt", 'r')
        imgUrlList = [i.rstrip() for i in p.readlines()]
        p.close()
    except Exception:
        imgUrlList = []

    with open("./imgUrl.txt", 'w' if not os.path.exists("./imgUrl.txt") else 'a') as f:
        for keyword in tqdm(keywords, desc="네이버 사진 수집 완료", position=0):
            ...
            for img in tqdm(imgs, desc=f"{keyword} 데이터 수집중", position=1):
                ...
                if imgUrl not in imgUrlList:  # 사진 경로가 중복이 아닌 경우에만 다운로드를 진행한다.
                    f.write(imgUrl+"\n")
                    imgUrlList.append(imgUrl)
                    saveImage(imgUrl, keyword, idx, forderPath)
                    idx += 1
        f.close()
    print(f"현재까지 {len(imgUrlList)}장 다운로드 됨")
